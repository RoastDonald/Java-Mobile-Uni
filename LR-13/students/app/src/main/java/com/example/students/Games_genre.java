...
public class Games_genre {
    ...
    public void httpAddAuthor(){
        Games_genre Author = null;

        JSONObject data = new JSONObject();
        try {
            data.put("author", author);
            data.put("genre", genre);
            data.put("countryOfIssue", countryOfIssue);
            data.put("criticallyTestedFlg", criticallyTestedFlg?1:0);
            data.put("onSaleFlg", onSaleFlg?1:0);
        }catch (JSONException e){
            e.printStackTrace();
        }
        String res = new HttpDataPoster(
                "http://10.0.2.2/?action=add_author", data
        ).getData();
        System.out.println(res);
    }
    public void httpSaveAuthor(){
        Games_genre Author = null;
        ...
        String res = new HttpDataPoster(
                "http://10.0.2.2/?action=edit_author", data
        ).getData();
        System.out.println(res);
    }
    public static void httpRemoveAuthor(int id){
        JSONObject data = new JSONObject();
        try{
            data.put("id", id);
        } catch (JSONException e) {
        ...
        String res = new HttpDataPoster(
                "http://10.0.2.2/?action=remove_author", data
        ).getData();
    }
